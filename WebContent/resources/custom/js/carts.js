...
function createModalCart(){
	$.ajax({
		...
		success: function(data){
			var exist = data.exist;
			if (exist == 0){
				//$('tbody#cartlines').replaceWith('<tbody id="cartlines"><tr><th>Il carrello � vuoto</th></tbody>');
...
function cartJson(data)  {
	...
	$.each(cartlines,function(i,item){
		products += '<tr id=' + i + '>' +
					...
					+ '</tr>';
		tot += parseFloat(item.lineTotal); /* trasforma in float perché @JsonSerialize nel modello passa una stringa, non un float */
	});
